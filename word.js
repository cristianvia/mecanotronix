...
export default class Word {
    ...
    init(word) {
        ...
        //if DOM element created, change its innerText
        if (this.el) {
    ...
    resetStartPosition() {
        ...
        this.angle = helper.random(0, 180);

        //some trigonometry
        const x = Math.sin(this.angle * (Math.PI / 90)) * helper.radius;
        const y = Math.cos(this.angle * (Math.PI / 90)) * helper.radius;

        //move element to x,y
        this.elTransform = `translate(${x}px, ${y}px)`;
        //rotate to target
        this.elTransform += `rotate(${90 - this.angle}deg)`;
    ...
    attack() {
        const sequence = [{
            ...
            offset: 1,
            transform: `rotate(${90 - this.angle}deg) translate(0,0)`
        }];
